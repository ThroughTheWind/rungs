...
| --- | --- | --- | --- | --- | --- | --- |
| F-018 | medium | next | Self-test fixtures still disagree with their engines when the runner is wired in, and **the set changes as the harness changes** — so the runner stays unwired and no fixture is executed on a normal run | Triaged 2026-08-16 by [WI-056](items/WI-056-triage-selftest-mismatches.md), which took the mismatches from 17 to 7 by fixing four **real** defects rather than the harness: an orphaned `[sections]` table nothing declared, two fixtures labelled for the wrong gate, a `reciprocal` rule configured and unimplemented, and a `non_empty` check that read any section with subsections as empty. The remaining 7 are `gates-frontmatter-valid` ×1, `findings-disposition-has-reason` ×2, `adr-required-fields` ×1, `session-sections-present` ×1, `skills-spec-pure` ×2 | Before anyone relies on fixtures being executed. Every round so far has found something real, so this is worth continuing rather than abandoning — but each round also moved which fixtures fail, and a gate whose failures move under it cannot be trusted to block a merge | Stop triaging one at a time. The recurring cause is that a fixture describes a *fragment* while an engine needs a *scenario* — sibling files, a registry, a git state. Give the format a `setup` block that names what the fixture assumes, which is an [ADR-0003](../decisions/ADR-0003-module-definition-format.md) change, then re-run and wire it in |
