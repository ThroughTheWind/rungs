...
| F-005 | high | now | **A markdown file containing any `{{token}}` has *every* link exempted from `gates-links-resolve` and `gates-paths-exist`.** `linkIntegrity` skips the whole file on the first token ([`src/engines.ts:155`](../../src/engines.ts)), so the documents most likely to discuss paths — the ones explaining templating — are exactly the ones never checked | Reproduced twice 2026-08-15 during WI-006/WI-007, in the same directory. `docs/backlog/items/WI-006-….md` mentions `{{repo.dirname}}`, so a genuinely broken `](../design/parameters.md)` gave **20 pass · 0 fail**. `WI-007-….md` has no token, so an identically-shaped broken `](../getting-started.md)` failed both gates at once. The site's independent `check:links` caught both. **16 non-excluded files are currently exempt**, including `modules/README.md`, ADR-0003, `docs/design/parameters.md`, and eight skills and rules that ship to consumer repos | Now. It ships in the `gates` module, so every scaffolded repo believes its links are checked; and the exemption is invisible — a green gate and a skipped file look identical | The heuristic's own comment says it exists for `modules/*/fragments/AGENTS.md` linking `{{path}}/README.md` — but `link_integrity.exclude` **already excludes `modules/*/fragments/**` and `modules/*/files/**` by path**, so it is likely redundant. Try deleting line 155 and see what genuinely breaks; if some files still need it, skip the individual *link* that contains a token, never the whole file. Assert both directions in the gate's self-test |
