...
| --- | --- | --- | --- | --- | --- | --- |
| F-023 | low | someday | `release-version-consistent` compares `package.json` with `*/package.json` and requires `all-agree`, which is wrong for a repo whose sibling package is deliberately versioned independently. It is why this repo registers only the fragment gate from the module it authors | On this tree 2026-08-17, root `package.json` is `0.2.0` and `site/package.json` is `0.0.1` — the docs site, a separate package that is never published. Installing the gate here would fail on a correct layout. Not measured on any consumer repo | Before a consumer with a monorepo installs `release`, or if the same shape is reported | Probably a `sources` default that does not glob siblings, or an `exclude`. The judgement — which packages share a version — is the repo's, so the fix is to let it be stated rather than to guess it. Do not silently drop `all-agree`: the gate's value is that it needs no opinion about which file is right |
