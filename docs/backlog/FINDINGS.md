...
| F-011 | medium | next | The `Console` component takes `date` and `source`, which it renders as the literal label **`REAL OUTPUT · <command>`**, and nothing checks that it ever was. Two of the three blocks on the landing page were fabricated | Found 2026-08-16 during WI-040. Both hero consoles carried `date={SITE.asOf}` and a `source` command. The `doctor` block showed three defect lines (*"this rule says MANDATORY and has no gate"*) that `doctor` does not emit — verified by running it against a fresh repo, which prints module states and counts. The `add concurrency` block showed an interactive `Install anyway? [y/N]`; the real path is non-interactive (`src/cli.ts:228-231`, *"Skipped — pass --confirm-threshold"*). The external reviewer read the first block as shipped capability and built a recommendation on it | Now — WI-040 corrected these three blocks by hand, so the register is clean and the mechanism that let them drift is not. The next console added has the same hole | A gate over `site/` that requires every `Console` with a `source` to cite a captured-output fixture, and re-runs the fixture. Cheaper interim: rename `source` to something that does not imply provenance for hand-written blocks, and reserve the current attribute for captured output |
| F-012 | low | next | [`README.md`](../../README.md) says `doctor` quotes a gate's incident back when it has never fired, and [ADR-0005](../decisions/ADR-0005-self-instrumentation.md) Tier B specifies `doctor`. The implementation prints it from `check` | Observed 2026-08-16: `npm run rungs -- check` ends with a *"Ledger questions (3540 recorded runs)"* block naming three never-fired gates; the same run of `doctor` prints no ledger output. The README's illustrative quote also names `check-findings-register`, which is not a gate id in this repo — the register gates are `findings-ids`, `findings-disposition-has-reason`, `findings-self-declared-closure` | When Tier B is next touched, or before an outside contributor reads ADR-0005 and cannot find the behaviour it specifies | Decide which command owns it — `check` is arguably better, since that is where a gate's silence is observed — then correct the other two surfaces. Whichever way, the ADR and the README must name the same command, and the example should use a real gate id |
| F-013 | medium | next | `astro dev` serves no stylesheet on Windows: every CSS request 403s, so the site is unstyled and cannot be visually reviewed locally. The production build is unaffected | Found 2026-08-16 verifying WI-040. `site/astro.config.mjs:10` computes the Vite `fs.allow` root as `new URL("../", import.meta.url).pathname`, which on Windows returns `/C:/Development/Repositories/ai-cli/` — a leading slash Vite then resolves against the drive. The dev log names the resulting root literally: `- C:/C:/Development/Repositories/ai-cli`. Every request for `src/design-system/styles.css` is refused as *"outside of Vite serving allow list"*; the browser console showed 49 consecutive 403s on one page load. `npm run build` and `npm run check` pass, and `dist/_astro/*.css` is emitted, because the content layer reads via fs at build time | Now, for anyone developing the site on Windows — it makes local visual review impossible, which is how a styling regression reaches production unseen | `fileURLToPath(new URL("../", import.meta.url))` instead of `.pathname`. One line, and it is the documented cross-platform form; `.pathname` on a `file:` URL is only path-correct on POSIX |
