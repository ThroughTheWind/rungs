...
| --- | --- | --- | --- | --- | --- | --- |
| F-019 | medium | next | `extensions_allowed_from` is declared in the skills gate table and implemented nowhere — and **`[skills.<name>]` is not parsed into the manifest at all**, so the opt-in that is meant to attach a portability cost to a decision attaches to nothing. Any Claude Code extension in a SKILL.md is simply a non-spec key | Found 2026-08-16 during [WI-057](items/WI-057-selftest-setup.md) by executing `skills-spec-pure`'s fixtures: the `pass` fixture — `disable-model-invocation` with `opted_in = ["disable-model-invocation"]` — fired `non-spec key 'disable-model-invocation'`. `grep -n "extensions_allowed_from\|extensions" src/engines*.ts` → no hits; `grep -n skills src/types.ts` → no hits, so `Manifest` has no `skills` field | When a module next opts a skill into an extension, or before an outside contributor reads `modules/README.md` §4 and believes the mechanism exists. It is the **fifth** instance of a rule configured and unread (F-007, `backticked_paths`, `reciprocal`, the orphaned `[sections]` table) | Parse `[skills.<name>].extensions` into the manifest, then have `frontmatter-schema` treat a non-`allowed` key as legal when the owning module opted in. The three fixtures already state the expected behaviour in both directions, so the assertion exists before the code does |
| F-018 | medium | next | The self-test runner is **sound**; the code that wires it into `gates-self-tests-both-directions` is not. Fixtures that pass when the runner is called directly are reported as mismatches through the gate, so the runner stays unwired and no fixture executes on a normal run | Narrowed 2026-08-16 by [WI-057](items/WI-057-selftest-setup.md) from 7 mismatches to 3, then localised: `session-sections-present`'s two fixtures return **`ok`, `ok`** when `runSelfTests` is called directly with the same table and blocks, and **mismatch** through `gateMeta`. Same inputs, different answer, so the defect is in the integration — table selection, substitution, or block filtering — not in the fixtures or the engines | Before wiring. This is now a bounded debugging task against a known-good reference: the direct call is the oracle, and the wiring must be made to agree with it | Diff what `gateMeta` passes to `runSelfTests` against what the direct call passes, for `session-sections-present` specifically, and fix the wiring. **Do not** add a `setup` block — the earlier recommendation to do so was wrong, see below |
