...
| F-009 | high | next | The site lock resolves Astro and transitive Sharp versions covered by two high-severity advisories, plus a low-severity esbuild advisory | `cd site && npm audit --json`, 2026-08-15: `astro` is direct and reported high, `sharp` is transitive and reported high, `esbuild` is transitive and reported low; 3 total. The audit proposes Astro 7.2.2 as a semver-major fix | Before the next site release or any use of the affected development/server paths | Upgrade Astro and compatible integrations to a fixed supported version, refresh the lock, then rebuild and run `npm audit`; if the static deployment makes an advisory inapplicable, dismiss only with that boundary and reason recorded |
| F-010 | low | next | Astro's content loader warns that duplicate collection ids are being overwritten, so a successful site build does not prove each source document has a unique rendered identity | `cd site && npm run build`, 2026-08-15 after WI-028 and again during WI-029: warnings for `backlog/items/wi-027-extract-dspy`, `backlog/items/wi-028-follow-on-research-synthesis`, `backlog/items/wi-029-apply-framework-patterns-to-modules`, `backlog/findings`, `design/module-catalog`, and `research/follow-on/synthesis`; `npm run check` still reports 0 errors and 0 broken links | Before relying on collection ids for navigation, search, or generated indexes, or when the content glob/schema changes | Make the docs input glob produce one source per id (or derive a collision-free id), then make the build fail or explicitly report on collisions; retain the route/link check separately |
