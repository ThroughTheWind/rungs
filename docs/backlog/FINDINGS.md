...
| --- | --- | --- | --- | --- | --- | --- |
| F-015 | medium | next | [`docs/backlog/README.md` §8](README.md) tells contributors to archive finished items with `rungs backlog archive`, and **there is no `backlog` command**. 39 `done` items sit in `items/`; `archive/` holds only its README | Found 2026-08-16 during WI-044, asked to close out finished items. `node src/cli.ts backlog archive` → *"unknown command: backlog"*; the nine commands in `--help` are `init doctor add check render upgrade eject setup modules`. `grep -l "^status: done" docs/backlog/items/*.md` → 39; `ls docs/backlog/archive/` → `README.md` | Before the first outside contributor follows §8 and finds nothing, or when `items/` gets large enough that "only work that can still change" stops being true. Same class as F-012 — a surface promising behaviour the code does not have | Either implement `rungs backlog archive` as §8 describes (it must recompute links repo-wide, which is the hard half and the reason it should not be done by hand), or change §8 to describe a manual move and own that. Do not archive 39 items by hand meanwhile: a repo-wide link rewrite that goes wrong is silent |

## Closed — 2026-08-16 by [WI-044](items/WI-044-resolve-open-findings.md)
