...
No CLI/module code changed. The implementation recommendations remain proposals; this item
delivers evaluation and its user-requested execution handoff.
...
5. The [remediation prompt](../../design/existing-promises-remediation-prompt.md) was independently
   reviewed for existing-promise scope and execution boundaries. It includes integrated lifecycle
   verification so later hooks, engines and metadata cannot silently invalidate ejection.

The user approved merge on 2026-09-05. Status moves to `done` and the item is archived in the merge
commit that integrates `docs/WI-084-tool-evaluation` into `main`. The inherited-failure regression
passed its one selected test; pre-merge gates passed all 30 and site checks reported zero diagnostics
and broken links. The remediation prompt is prepared for a subsequent task and has not been executed.
