...
| Docs and frontend | `cd site && npm run build` → 97 pages; `cd site && npm run check` → 0 Astro errors/warnings/hints, 1,203 internal links, 0 broken | accepted-risk | The build retains the pre-existing duplicate-content-id warnings; see F-010 / WI-034. |
...
| Platform matrix | **Superseded 2026-08-17.** At assessment time only Windows PowerShell + Node 22.22.3 was exercised. CI now runs `{ubuntu, macos, windows} × node {22.18, 22}` on every push, and its first runs found **four defects that had never worked off Windows** — [F-033](../backlog/FINDINGS.md) (a gate shipped in four of five profiles) and F-036 (three more, one shared path bug). The row's disposition was `not-applicable`; it should have been blocking, and it took twelve days and a published package to find that out | blocking → addressed | The matrix is the criterion now, not a note about one. `release-runbook.md` §6 requires reading the run before the tag. |
