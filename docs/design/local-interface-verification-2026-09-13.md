...
in `fa23eb5`; the final presentation correction in `f48a76b` accounts for the browser scrollbar in a
narrow dialog and announces job status. It changes no runner, pin, process or install semantics.
Origin was fetched again before landing: local and remote main both resolved to `d505f99`, with no
intervening product change. Final closeout changes only evidence, status and archive links.
...
| Final inspection/server cases | `node --max-old-space-size=2048 --test --test-concurrency=1 test/ui-inspection.test.js test/ui-server.test.js` | 26 pass, 0 fail; 23.61s |
...
| --- | --- | --- | --- |
| Compressed package (`npm pack --dry-run --json --ignore-scripts`) | 524,159 bytes | 633,914 bytes | Growth < 307,200 bytes; observed growth 109,755 bytes |
...
A second disposable repository was created through `rungs init <path> minimal` and opened with
`--read-only`. With no Git, the browser showed the two installed modules, matching 0.5.0 pin, zero
work items and eleven registered declarations. Preparing a check plan selected nine runner gates,
named read-only mode as the refusal reason and disabled **Run checks**. Temporarily moving
`.ai/rules/README.md` out of its installed location produced the explicit missing-file entry in the
inspection result; restoring it completed the partial-install probe. No gate command ran in this
repository during browser inspection.
