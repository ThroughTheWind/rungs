...
import { loadRegistry, loadTable } from './check.ts';
import { selectEngineTable } from './engine-table.ts';
import { mergeBlock } from './substitute.ts';
...
export function render(repoRoot: string, harnesses: Harness[]): RenderEntry[] {
  ...
  renderDerivedBlocks(repoRoot);
  return prepared.entries;
}

/**
 * Managed blocks a gate table declares as **derived** from files — today the
 * `file-index` shape: one table row per matching source, with the frontmatter
 * columns the table names.
 *
 * `adr-index-current` has checked for the `adr-index` block since the module
 * shipped, and `render` had never written anything into it; the block stayed
 * empty beside ten records while a hand-kept table did the index's job below
 * it. WI-087 made the gate compare rows to sources, which would have turned
 * every consumer's index red with no command that could fix it — so the
 * derivation the gate always assumed is implemented here, in the command the
 * gate's message names.
 */
export function renderDerivedBlocks(repoRoot: string): string[] {
  const written: string[] = [];
  const files = walk(repoRoot);
  for (const gate of loadRegistry(repoRoot).gates) {
    if (gate.engine !== 'render-freshness' || !gate.table) continue;
    const table = loadTable(gate.table, repoRoot);
    if (!table) continue;
    let specs: any;
    try {
      specs = selectEngineTable(table, 'render-freshness', gate.id);
    } catch {
      continue;
    }
    for (const spec of Array.isArray(specs) ? specs : [specs]) {
      if (spec?.derives !== 'file-index' || !spec.block?.file || !spec.block?.marker || !Array.isArray(spec.sources)) continue;
      const target = preflightEmittedPaths(repoRoot, [{ moduleName: gate.module ?? 'render', target: spec.block.file, writeExisting: true }])[0];
      if (!existsSync(target.absolute)) continue;
      const columns: string[] = Array.isArray(spec.columns) && spec.columns.length ? spec.columns.map(String) : ['id', 'title', 'status', 'date'];
      const sources = [...new Set(spec.sources.flatMap((p: string) => matchAny(files, p)))].sort();
      const blockDir = spec.block.file.split('/').slice(0, -1).join('/');
      const rows = sources.map((rel) => {
        const raw = semanticText(readFileSync(join(repoRoot, rel), 'utf8'));
        const fm = raw.match(/^---\n([\s\S]*?)\n---/)?.[1] ?? '';
        const field = (k: string) => fm.match(new RegExp(`^${k}:\\s*(.+)$`, 'm'))?.[1].trim().replace(/^["']|["']$/g, '') ?? '';
        const href = blockDir && rel.startsWith(`${blockDir}/`) ? rel.slice(blockDir.length + 1) : rel;
        return `| ${columns.map((c, i) => (i === 0 ? `[${field(c) || rel}](${href})` : field(c))).join(' | ')} |`;
      });
      // Zero sources render the same `— · none yet` placeholder the shipped template carries, so a
      // fresh scaffold and its first `render` agree byte for byte and the freshness gate, which
      // skips that row, counts zero rows for zero files (WI-091).
      const placeholder = `| — | *none yet* |${' |'.repeat(Math.max(columns.length - 2, 0))}`;
      const fragment = [
        `<!-- rungs:begin ${spec.block.marker} -->`,
        `<!-- Generated by \`${spec.command ?? 'rungs render'}\` from ${spec.sources.join(', ')}. Do not edit — rerun it to regenerate. -->`,
        `| ${columns.join(' | ')} |`,
        `| ${columns.map(() => '---').join(' | ')} |`,
        ...(rows.length ? rows : [placeholder]),
        `<!-- rungs:end ${spec.block.marker} -->`,
      ].join('\n');
      const existing = readFileSync(target.absolute, 'utf8');
      const merged = mergeBlock(existing, fragment, spec.block.marker);
      if (merged !== existing) {
        writeFileSync(target.absolute, merged);
        written.push(spec.block.file);
      }
    }
  }
  return written;
